/*
Biran Shah
This program opens and checks the ciphered text and key files to ensure both of its validity, and then sends them to the server, otp_dec_d.
...This program than stdouts the output of the original message
 */
...
//	printf("key buffer is: %s\n", keyBuffer);	
//	printf("plain buffer is: %s\n", plainBuffer);
...
//printf("CLIENT: buffer length: %d\n", bufferLength);
    ...
    char packets[10000]; 
    const char ch = '*';
    if((strchr(secretCode,ch)==NULL))   //if sequence not found
    {
        do  //keep receiving bits of the incoming sequence
        {
            memset(packets, '\0', sizeof(packets));  
            charsRead = recv(socketFD, packets, sizeof(packets)-1,0);
            strcat(secretCode, packets);

            if (charsRead < 0) error("ERROR reading from socket"); 

        }   while(strchr(secretCode,ch) == NULL);   //once * found, message has been received
    }
